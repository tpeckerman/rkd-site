 ...
 * for the full walkthrough. Copy the deployed /exec URL into:
 *   - tournament.html            (GOOGLE_SCRIPT_URL)
 *   - rkd-air-hockey src/routes/index.tsx (GOOGLE_SCRIPT_URL)
 *
 * Each form submission POSTs a "form" value ("signup" or "updates") plus
 * "name" and "phone". Rows are appended below the existing header row, so
 * data always starts on row 2.
 */
